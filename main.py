    ...
    while True:
        ...
        for account in data['accounts'][index:]:
            ...
            for case in differences:
                ...
                #             tweet = f"{account['name']} (https://twitter.com/{account['username']}) unfollowed {item.t1['name']} (https://twitter.com/{item.t1['username']}) 📉"
                #             send_tweet(tweet)
                #             telegram_bot_sendtext(tweet)
                #             print("send remove")
                if case == 'dictionary_item_added':
                    for item in differences[case]:
                        if item.t2['username'] not in blacklist:
                            tweet = f"{account['name']} (https://twitter.com/{account['username']}) followed {item.t2['name']} (https://twitter.com/{item.t2['username']}) 📈"
                            # send_tweet(tweet)
                            telegram_bot_sendtext(tweet)
